...
{
    private function welcome(?string $service = null): void
    {
        ...
        $this->comment("lego make:feature LoginUser $service");
        if ($service) {
        ...
        $this->comment('lego make:job GetUserByEmail User');
        $this->info('will generate <fg=cyan>app/Domains/User/Jobs/GetUserByEmailJob.php</>');
        $this->info('');
        $this->info('For more Job examples check out Lego\'s built-in jobs:');
        $this->comment('- Lego\Domains\Http\Jobs\RespondWithJsonJob');
        $this->info('for consistent JSON structure responses.');
        $this->info('');
        $this->comment('- Lego\Domains\Http\Jobs\RespondWithJsonErrorJob');
        $this->info('for consistent JSON error responses.');
        $this->info('');
        $this->comment('- Lego\Domains\Http\Jobs\RespondWithViewJob');
        $this->info('basic view and data response functionality.');

        $this->info('');

        $this->info('Finally you can group multiple jobs in an operation');
        $this->comment("lego make:operation ProcessUserLogin $service");
        ...
        $this->info('For more details, help yourself with the docs at https://docs.legoravel.dev');
        $this->info('');
